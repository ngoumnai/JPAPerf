...
public class EventResource {
    ...
    @Path("date/{date}")
    @Produces({"application/json"})
    public List<EventDTO> getEventByDate(String date) throws ParseException {
        List<EventDTO> eventsDTO = new ArrayList<EventDTO>();
        List<Event> events = serviceBean.findEventByDate(date);
        if(events == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
    ...
    @Produces({"application/json"})
    public List<EventDTO> getEventByType(String eventType) throws ParseException {
        List<EventDTO> eventsDTO = new ArrayList<EventDTO>();
        List<Event> events = serviceBean.findEventByType(eventType);
        if(events == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Event event: events){
            EventDTO eventDTO = new EventDTO();
