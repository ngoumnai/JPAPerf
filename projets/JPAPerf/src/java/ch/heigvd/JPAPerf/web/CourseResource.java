...
public class CourseResource {
    ...
    public CourseDTO getCourseByName(@PathParam("name") String name) {
        CourseDTO dto = new CourseDTO();
        Course course = serviceBean.findCourseByName(name);
        if(course == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        dto.setDescriptor(course);
        return dto;
    ...
    public List<CourseDTO> getCoursesBySemester(@PathParam("semester") int semester) {
        ...
        List<Course> courses = serviceBean.findCourseBySemester(semester);
        if(courses == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
    ...
    public List<CourseDTO> getCoursesByPeriod(@PathParam("period") int period) {
        List<CourseDTO> coursesDTO = new ArrayList<CourseDTO>();
        List<Course> courses = serviceBean.findCourseByPeriod(period);
        if(courses == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Course course: courses){
            CourseDTO courseDTO = new CourseDTO();
 ...
 //   @Path("periodInRange/{min, max}")
    @Produces({"application/json"})
    public List<CourseDTO> getCoursesByPeriodInRange(int min, int max) {
        List<CourseDTO> coursesDTO = new ArrayList<CourseDTO>();
        List<Course> courses = serviceBean.findCourseByPeriodInRange(min, max);
        if(courses == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Course course: courses){
            CourseDTO courseDTO = new CourseDTO();
