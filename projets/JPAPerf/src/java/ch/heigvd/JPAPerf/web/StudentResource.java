...
public class StudentResource {
    ...
    public StudentDTO getStudentByName(@PathParam("name") String name) {
        StudentDTO dto = new StudentDTO();
        Student student = serviceBean.findStudentByName(name);
        if(student == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        dto.setDescriptor(student);
        return dto;
    ...
    public List<StudentDTO> getStudentByMajor(@PathParam("major") String major) {
        ...
        List<Student> students = serviceBean.findStudentByMajor(major);
        if(students == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
    ...
    public List<StudentDTO> getStudentByPeriod(@PathParam("period") int period) {
        List<StudentDTO> studentsDTO = new ArrayList<StudentDTO>();
        List<Student> students = serviceBean.findStudentByPeriod(period);
        if(students == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Student student: students){
            StudentDTO studentDTO = new StudentDTO();
 ...
 //   @Path("periodInRange/{min, max}")
    @Produces({"application/json"})
    public List<StudentDTO> getStudentByPeriodInRange(int min, int max) {
        List<StudentDTO> studentsDTO = new ArrayList<StudentDTO>();
        List<Student> students = serviceBean.findStudentByPeriodInRange(min, max);
        if(students == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Student student: students){
            StudentDTO studentDTO = new StudentDTO();
    ...
    public List<StudentDTO> getBusyStudent() {
        List<StudentDTO> studentsDTO = new ArrayList<StudentDTO>();
        List<Student> students = serviceBean.findBusyStudent();
        if(students == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Student student: students){
            StudentDTO studentDTO = new StudentDTO();
    ...
    public List<StudentDTO> getFreeStudent() {
        List<StudentDTO> studentsDTO = new ArrayList<StudentDTO>();
        List<Student> students = serviceBean.findFreeStudent();
        if(students == null)
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        for(Student student: students){
            StudentDTO studentDTO = new StudentDTO();
